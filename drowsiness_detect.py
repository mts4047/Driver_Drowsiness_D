...
while cap.isOpened():
    ...
    if result.face_landmarks:
        ...
        # print('I am happy')   #for Debugging
        mouth_input = preprocess_mouth(mouth_roi)
        # print("Debugging",mouth_input.shape, '\n')  # Debugging line to check the shape of mouth_input

        if mouth_input is not None:
            pred = mouth_model.predict(mouth_input, verbose=0)[0][1]
            # pred = mouth_model.predict(mouth_input, verbose=0)
            # pred_indices = np.argmax(pred, axis=1)  # Get the predicted class index
            # final_pred = pred_indices[0]  # Get the confidence of the predicted class
            # mouth_history.append(final_pred)
            mouth_history.append(pred)

        smooth_mouth = np.mean(mouth_history) if mouth_history else 0
        print(f"Debugging: smooth_mouth={smooth_mouth:.4f}")  # Debugging line to check the value of smooth_mouth
